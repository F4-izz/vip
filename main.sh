...
ALLOWED_IP=$(curl -sS https://raw.githubusercontent.com/f4-izz/izin/main/ip | grep -wE "$MYIP" | awk '{print $4}')
...
curl -sL ip.dekaa.my.id > /etc/xray/ipvps
...
printf "q\n" | rclone config
wget -qO /root/.config/rclone/rclone.conf "https://drive.google.com/u/4/uc?id=1g8cdEeC-Ev_dQKaY2GajKG8p6zWpwjbR&export=download"
cd /bin
...
wget ${REPO}menu/update1.sh && chmod +x update1.sh && ./update1.sh
}
function profile(){
clear
